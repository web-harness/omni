...
async function getDb(threadId: string): Promise<Database> {
  if (DBS.has(threadId)) return DBS.get(threadId)!;
  ...
  await persistDb(threadId, db, dbPath);
  DBS.set(threadId, db);
  return db;
}

async function persistDb(threadId: string, db: Database, dbPath: string): Promise<void> {
...
export class SqlJsSaver implements BaseCheckpointSaver {
  ...
  async put(
    ...
    await persistDb(threadId, db, `/home/checkpoints/${threadId}.sqlite`);
