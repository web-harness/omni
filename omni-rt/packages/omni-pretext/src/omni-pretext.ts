...
function doTruncate(
  ...
  if (pretext.layout(pretext.prepare(text, font), width, lh).lineCount <= maxLines) return text;
  let lo = 0;
  let hi = text.length;
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    const candidate = text.slice(0, mid) + "\u2026";
    if (pretext.layout(pretext.prepare(candidate, font), width, lh).lineCount <= maxLines) lo = mid;
    else hi = mid - 1;
  }
...
function findFitSize(
  ...
  for (let size = startSize; size > minSize; size -= 0.5) {
    ...
    if (pretext.layout(pretext.prepare(text, font), width, lh).lineCount <= maxLines) return size;
  }
  return minSize;
}
...
class OmniText extends HTMLElement {
  ...
  private async _render() {
    ...
    if (strategy === "truncate") {
      const result = doTruncate(pretext, text, font, width, maxLines, baseLH);
      ...
      const size = findFitSize(pretext, text, style, weight, family, baseFontSize, minSize, width, maxLines, baseLH);
      ...
      const result = doTruncate(pretext, text, shrunkFont, width, maxLines, shrunkLH);
      if (this.textContent !== result) this.textContent = result;
      if (result !== text) this.setAttribute("title", text);
      else this.removeAttribute("title");
    } else {
