...
class OmniPopper extends LitElement {
  ...
  private async show() {
    ...
    contentEl.setAttribute("data-show", "");

    const [ox, oy] = this.offset.split(",").map(Number);
    const popper = await loadPopperModule();
    if (!this.open || !this.isConnected || !this.contentEl || !this.triggerEl) return;

    if (this.popperInstance) {
      this.popperInstance.destroy();
    }
    this.popperInstance = popper.createPopper(this.triggerEl as HTMLElement, this.contentEl, {
